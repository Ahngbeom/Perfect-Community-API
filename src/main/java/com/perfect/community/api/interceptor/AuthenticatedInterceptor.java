...
//					return;
        ...
        /* By Authenticated User */
            ...
            if (HttpMethod.GET.matches(requestData.METHOD)) {
                ...
                if (requestData.URI.equals("/api/post"))
                    return;
                else if (requestData.URI.startsWith("/api/post/scrap") && requestData.PATH_VARIABLES.containsKey("postNo"))
                    return;
            } else if (HttpMethod.PUT.matches(requestData.METHOD)) {
                return;
            } else if (HttpMethod.PATCH.matches(requestData.METHOD)) {
                ...
                if (requestData.PATH_VARIABLES.containsKey("postNo") &&
                        (postService.isWriter(Long.parseLong(requestData.PATH_VARIABLES.get("postNo")), requestData.AUTHENTICATION.getName()))
                        || requestData.IS_ADMIN)
                    return;
            }
            throw new AuthenticationServiceException("Access denied");
        }
